...
//                          par elle-même : seul un SMS de réception VÉRIFIÉ
//                          (_rapprochement_lib.php) peut la servir sans humain.
//   POST ?action=preuve    PUBLIC — l'acheteur colle l'ID de transaction de son
//                          SMS { ref, txid, tel4 }. N'accorde rien non plus.
...
require_once __DIR__ . '/_manuel_lib.php';    // pmEtat(), pmOctroyer()… — partagés avec le rapprochement SMS
require_once __DIR__ . '/_rapprochement_lib.php'; // validation automatique par le SMS de réception
...
$dir = vrt_pm_dir();   // dossier + sentinelles .htaccess / index.php : _manuel_lib.php
...
if ($action === 'declarer' && $method === 'POST') {
    ...
    // Facultatif : l'ID de transaction du SMS de l'acheteur, s'il l'a déjà.
    $txid = strtoupper(preg_replace('/[^A-Za-z0-9.\-]/', '', (string) ($in['txid'] ?? '')));
    if (strlen($txid) >= 6 && strlen($txid) <= 40) $etat['txid'] = $txid;
    unset($etat['paid_at']);
    pmEcrire($path, $etat);
    pmLog('[DECLARE] ref=' . $ref . ' intent=' . $intent . ' montant=' . $mont . ' tel=…' . substr($telNu, -4)
        . ($parrainageEtat ? ' code_ami=1' : ''));

    /* Payé AVANT d'avoir déclaré (ou déclaré après coup) : le SMS vérifié est
       peut-être déjà là. Il sert la commande tout de suite, sans humain. */
    $auto = ['auto' => false];
    try { $auto = vrt_sms_rapprocher_commande(pmRefSafe($ref)); }
    catch (\Throwable $e) { pmLog('[SMS_ERR] ref=' . $ref . ' ' . $e->getMessage()); }

    pmOut(['ok' => true, 'declare' => true, 'ref' => $ref, 'montant' => $mont,
           'auto' => !empty($auto['auto']), 'rapprochementActif' => vrt_sms_actif()]);
}

// ════════════════════════════════════════════════════════════
// PREUVE — PUBLIC : l'acheteur colle l'ID de transaction de son SMS
// ════════════════════════════════════════════════════════════
// Utile quand il a payé d'un AUTRE numéro que celui déclaré (celui d'un
// parent) : le numéro ne peut plus rapprocher, l'ID de transaction si.
// N'ACCORDE RIEN : l'ID ne sert qu'à retrouver un SMS de réception que la
// chaîne des soldes a DÉJÀ vérifié. Réservée au détenteur de la commande
// (4 derniers chiffres du numéro déclaré), et une fois accordée, figée.
if ($action === 'preuve' && $method === 'POST') {
    $in   = json_decode((string) file_get_contents('php://input'), true) ?: [];
    $ref  = trim((string) ($in['ref'] ?? ''));
    $txid = strtoupper(preg_replace('/[^A-Za-z0-9.\-]/', '', (string) ($in['txid'] ?? '')));
    $tel4 = preg_replace('/\D+/', '', (string) ($in['tel4'] ?? ''));
    $refFic = pmRefSafe($ref);
    if ($refFic === '' || strlen($txid) < 6 || strlen($txid) > 40 || strlen($tel4) !== 4)
        pmOut(['error' => 'Référence, ID de transaction ou numéro invalide.'], 400);

    $rl = $dir . '_rl/';
    if (!is_dir($rl)) @mkdir($rl, 0750, true);
    if (!pmCompteur($rl . 'preuve_' . pmIpCle() . '_' . date('YmdH') . '.txt', 12))
        pmOut(['error' => 'Trop d’essais depuis cette connexion. Réessayez dans une heure.'], 429);

    $path = pmPath($refFic);
    $cmd  = pmLire($path);
    if (!$cmd || substr((string) ($cmd['clientTel'] ?? ''), -4) !== $tel4) pmOut(['error' => 'Commande introuvable.'], 404);
    if (!empty($cmd['granted'])) pmOut(['ok' => true, 'deja' => true, 'auto' => true]);

    $cmd['txid'] = $txid;
    pmEcrire($path, $cmd);
    pmLog('[PREUVE] ref=' . $ref . ' tx=' . $txid);
    $auto = ['auto' => false];
    try { $auto = vrt_sms_rapprocher_commande($refFic); }
    catch (\Throwable $e) { pmLog('[SMS_ERR] ref=' . $ref . ' ' . $e->getMessage()); }
    pmOut(['ok' => true, 'auto' => !empty($auto['auto'])]);
}
